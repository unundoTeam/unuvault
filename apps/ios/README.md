...
parse the Mac companion invite envelope and QR payload, reject expired,
malformed, or unsupported-endpoint payloads, build a target-device identity
claim with `deviceId`, `displayName`, and `publicKeyFingerprint`, post that
claim to the invite-provided Mac pairing endpoint without a bridge bearer token,
and parse the Mac handoff response envelope while rejecting invalid, expired,

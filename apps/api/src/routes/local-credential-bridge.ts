...
import { createConfiguredLocalCredentialBridgeService } from "../lib/supabase";
import {
  LocalCredentialBridgeCredentialNotFoundError,
  LocalCredentialBridgeUnauthorizedError,
  LocalCredentialBridgeValidationError,
} from "../services/local-credential-bridge-service";

export type LocalCredentialBridgeRouteDependencies = {
  accessToken?: string;
  service: {
...
export function createLocalCredentialBridgeRoutes(
  deps: LocalCredentialBridgeRouteDependencies,
): FastifyPluginAsync {
  return async (app) => {
    app.put("/credentials/unlocked-session", async (request, reply) => {
      ...
      try {
        ...
        if (error instanceof LocalCredentialBridgeUnauthorizedError) {
          reply.code(401);
          return {
            ok: false,
            error: "invalid_token",
        ...
        if (error instanceof LocalCredentialBridgeValidationError) {
          reply.code(400);
          return {
            ok: false,
    ...
    app.post("/credentials/release", async (request, reply) => {
      ...
      try {
        ...
        if (error instanceof LocalCredentialBridgeCredentialNotFoundError) {
          reply.code(404);
          return {
            ok: false,
...
export const localCredentialBridgeRoutes = createLocalCredentialBridgeRoutes({
  accessToken: process.env.UNUVAULT_BRIDGE_TOKEN,
  service: createConfiguredLocalCredentialBridgeService(),
});

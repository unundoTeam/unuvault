...
`job_id` is a canonical lowercase UUIDv4 generated by Postgres
`gen_random_uuid()`. Clients must reject a success payload unless the response
has exactly `job_id` and `status`, the id matches the UUIDv4 contract, and
`status` is exactly `recorded`.
...
- `400`: `{ "ok": false, "error": "invalid_import_report" }`, including malformed JSON
- `401`: exact `{ "ok": false, "error": "missing_bearer_token" }` or `{ "ok": false, "error": "invalid_token" }`
- `404`: `{ "ok": false, "error": "profile_not_found" }`
- `413`: `{ "ok": false, "error": "import_report_too_large" }`
- `415`: `{ "ok": false, "error": "unsupported_media_type" }`
- `500`: `{ "ok": false, "error": "import_report_create_failed" }`

Error responses contain exactly `ok: false` plus one allowlisted stable code;
unknown or extra keys are not part of the wire contract. Clients map malformed
success/error JSON, unknown codes, parse failures, and any response whose `ok`
flag is not the expected boolean to a static status-based fallback. No response
may expose request, identity-provider, Supabase, parser, or stack-trace details.

...
### Option 1: Upsert `changed_items` inside `POST /vault/sync` and then return the refreshed vault (Recommended)

- Keep a single sync endpoint
- Accept typed full-record `changed_items`
- Upsert those rows under the authenticated `users_profile`
...
- `apps/api/src/services/vault-service.ts` resolves the authenticated profile, upserts incoming items for that profile, then lists the current vault rows and projects them back into the existing sync response.
- `apps/api/src/lib/supabase.ts` adds a bulk upsert helper that always writes rows scoped to the authenticated `user_profile_id`.
...
- Existing item in another user's vault:
  - not touched
  - writes are always scoped by the authenticated `user_profile_id`
...
3. The vault service transforms each incoming item into a `vault_items` row for `profile.id`.
4. The Supabase adapter bulk-upserts those rows.
5. The service lists all `vault_items` for `profile.id`.
6. The API returns the current full vault as `updated_items`.
...
- A database failure during upsert or re-read returns `500 sync_failed`.
...
- Add Supabase adapter coverage for bulk upserting vault items under a given `user_profile_id`.
- Extend vault service tests to cover:
  - empty `changed_items` preserves current read-only behavior
  - new items are upserted and then returned

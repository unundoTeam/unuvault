...
| VoiceOver | Static labels cover both destinations, selected state, progress, recognized Mac, import, status, error, and recovery. Composition announcements and receive-flow labels are covered by the 72-test iOS gate; manual rotor path proof is not recorded yet. | partial |
...
product-composition and pairing v3 Pencil source frames, the current 72-test
Swift package gate, and four XcodeGen-backed simulator host screenshots. The
